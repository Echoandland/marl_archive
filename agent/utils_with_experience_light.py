...
class Agent:
    def __init__(self, instruction, role, examplers=None, model_info='gpt-5', port=8000, img_path=None, enable_experience=False):
...
class Group:
    def __init__(self, goal, members, model, question, port=8000, examplers=None, enable_experience=False):
        self.goal = goal
        self.members = []
        for member_info in members:
            _agent = Agent('You are a {} who {}.'.format(f"{member_info['specialty']} ({member_info['role']})", member_info['expertise_description'].lower()), role=f"{member_info['specialty']} ({member_info['role']})", model_info=random.choice(model), port=port, enable_experience=enable_experience)
        ...
        for m in members:
            ...
            for reviewer in members:
                ...
                peer_prompt = (
                    ...
                    "Guidelines (flexible):\n"
                    "- When the TARGET is the leader, usually place mild differences under 'agreements' as suggestions, and only record disagreements when there are clear, well-grounded contradictions (ideally with at least two distinct anchors). You may phrase these points naturally.\n"
                    # "- When the TARGET is not the leader, still give weight to the leader’s reasoning, and challenge only if you believe it meaningfully changes safety or diagnostic likelihood. Express the reasoning in your own words.\n"
                    "- Avoid repeating earlier review points. Try to bring in fresh, non-overlapping insights.\n"
                    "- Each agreement or disagreement should reference at least one [anchor: <feature>], but you can explain context more freely and at varying length.\n\n"

                    "Return STRICT JSON (no fences):\n"
                    '{{ "analysis": "自由描述，可多句，概述关键观点及推理。", '
                    '"agreements": ["可多句且更自由地描述同意或补充的要点，每条需有[anchor: <feature>]"], '
                    '"disagreements": null | ["可多句且更自由地描述不同意见或反驳，每条需有[anchor: <feature>]"] }}'
                )
...
def determine_multiagents(question, difficulty):
    if "adaptive" not in difficulty:
        if "cot" in difficulty or "zero-shot" in difficulty or "none" in difficulty:
            return 'cot'
        elif "multi-agent" in difficulty or "multi" in difficulty:
            return 'multi-agent'
    
    difficulty_prompt = (
        f"Given the medical query below:\n"
        f"{question}\n\n"
        f"Decide whether it can be directly solved by chain-of-thought reasoning (answer 'cot') "
        f"or requires collaboration of multiple medical experts (answer 'multi-agent').\n"
        f"Output only one word: 'cot' or 'multi-agent'."
    )
        
    medical_agent = Agent(instruction='You are a medical expert who conducts initial assessment and your job is to decide the reasoning method(cot/multi-agent) of the medical query.', role='medical expert', model_info='gpt-5')
    ...
    if 'cot' in response.lower():
        return 'cot'
    elif 'multi-agent' in response.lower():
        return 'multi-agent'

def diagnosis_prompt(patient_info: str, cot: str):
        # patient_info_type = "EHR" or "PHENOTYPE"
        system_prompt = "You are a specialist in the field of rare diseases."
        diagnosis_system_prompt = system_prompt + \
        ...
        "</top10>\n\n")
        if cot == "zero-shot":
            prompt += "Let us think step by step, you must think more steps and then give the top 10 diagnoses. "
        else:
            prompt += "The top  10 diagnoses are:"
        
        # elif cot == "none":
            
        return (diagnosis_system_prompt, prompt)

def process_cot(sample, question, model, result_file, args):
    system_prompt, prompt = diagnosis_prompt(question, "zero-shot")
    medical_agent = Agent(instruction=system_prompt, role='medical expert', model_info=model, port=args.port)
    
    final_decision = medical_agent.temp_responses(prompt)

    _, top10 = parse_top10_block(final_decision)
    ...
    result = {
        ...
        'raw_predict_decision': final_decision,
        'predict_rank': None
    }
...
def process_rareagent(sample, question, model, args, max_turn: int = 10, num_members: int = 4, result_file: str = 'result.json', interaction_history_file: str = 'interaction_history.json', enable_experience: bool = False):
    ...
    group_instance = Group(group_goal, members, model, question, args.port, enable_experience=enable_experience)
